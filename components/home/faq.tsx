...
export function FAQ() {
  ...
  return (
    <section
      id="faq"
      className="section-mesh relative overflow-hidden"
    >
      <GradientMesh />

      <div className="container-wide px-4 sm:px-6 lg:px-8 relative section-padding">
            ...
            <div className="relative overflow-hidden rounded-2xl premium-shadow">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-6 text-white">
                ...
                <Link
                  ...
                  className="mt-4 inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-xl bg-accent px-4 py-2.5 text-sm font-bold text-text transition hover:bg-accent/90"
                >
            ...
            >
              {faqs.map((faq, i) => (
                <AccordionItem
                  key={i}
                  value={`item-${i}`}
                  className="overflow-hidden rounded-2xl border border-slate-200/70 bg-white px-4 sm:px-6 shadow-sm transition-shadow hover:shadow-md"
                >
                  <AccordionTrigger className="text-left font-semibold text-text hover:no-underline break-words">
                    {faq.question}
                  </AccordionTrigger>

                  <AccordionContent className="leading-relaxed text-gray break-words">
                    {faq.answer}
                  </AccordionContent>
                </AccordionItem>
              ))}
