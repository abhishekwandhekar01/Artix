...
function BrochurePreview() {
  ...
  return (
    ...
    >
      <div className="border-b border-slate-100 bg-gradient-to-r from-primary/[0.07] via-white to-accent/[0.08] px-5 py-5 sm:px-7 sm:py-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            ...
            <h3 className="mt-3 text-xl font-extrabold text-text sm:text-2xl">{BROCHURE.title}</h3>
            <p className="mt-2 text-sm leading-relaxed text-gray sm:text-base">{BROCHURE.description}</p>
          </div>
      ...
      <div className="bg-gradient-to-b from-slate-100 to-slate-50/80 p-3 sm:p-5 lg:p-6">
        <div className="overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-inner">
          <iframe
            ...
            className="block h-[min(72vh,780px)] min-h-[420px] w-full bg-white sm:min-h-[520px]"
          />
        </div>
        <p className="mt-3 text-center text-xs text-gray">
          Scroll inside the viewer to read every page, or download the PDF for offline sharing.
        </p>
      </div>
    </motion.div>
  );
              ...
              >
                ...
                <div className="flex flex-1 flex-col p-5">
                  <div className="mb-2 flex items-center gap-2 text-primary">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-wider">Free Download</span>
                  </div>
                  <h3 className="font-bold text-text">{item.title}</h3>
                  <p className="mb-4 mt-1.5 flex-1 text-xs leading-relaxed text-gray">{item.description}</p>
