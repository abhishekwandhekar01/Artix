            ...
            <div className="rounded-2xl bg-white p-6 shadow-2xl lg:p-8">
              {submitted ? (
                <div className="py-12 text-center">
                  ...
                  <h3 className="text-xl font-bold">Demo Scheduled!</h3>
                  <p className="mt-2 text-sm text-gray">We&apos;ll contact you within 24 hours.</p>
                </div>
              ) : (
                <form onSubmit={handleSubmit(async (d) => { await new Promise(r => setTimeout(r, 900)); console.log({...d, selectedModules}); setSubmitted(true); })} className="space-y-4">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div><Label>Name *</Label><Input {...register("name")} className="mt-1" placeholder="Dr. Priya" /></div>
                    <div><Label>Clinic *</Label><Input {...register("hospital")} className="mt-1" placeholder="LifeSpring IVF" /></div>
                  </div>
                  <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {demoFormModules.map(m => (
                      <button key={m} type="button" onClick={() => setSelectedModules(p => p.includes(m) ? p.filter(x => x !== m) : [...p, m])}
                        className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${selectedModules.includes(m) ? "bg-primary text-white" : "border border-slate-200 text-gray"}`}>{m}</button>
                    ))}
                  </div>
                  <div><Label>Message</Label><Textarea {...register("message")} className="mt-1" rows={2} /></div>
                  <Button type="submit" size="lg" className="w-full" disabled={isSubmitting}>
                    {isSubmitting ? <><Loader2 className="animate-spin h-4 w-4" /> Sending...</> : "Schedule My Free Demo"}
                  </Button>
                </form>
              )}
            </div>
